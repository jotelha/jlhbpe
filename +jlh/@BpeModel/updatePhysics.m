function obj = updatePhysics(obj)
...
%     pdePbulkDirichletBC = '0';
    pdePbulkDirichletBC = 'smoothenBpeBC(x)*phi_s';
...
%     nxTerm = prepTerm('(-D_id/D_ref*(cx_id+z_id*c_id*phix))','D_id','cx_id','z_id','c_id',obj.D_id, obj.cx_id,obj.z_id,obj.c_id);
%     nyTerm = prepTerm('(-D_id/D_ref*(cy_id+z_id*c_id*phiy))','D_id','cy_id','z_id','c_id',obj.D_id, obj.cy_id,obj.z_id,obj.c_id);
    nxTerm = prepTerm('(-cx_id-z_id*c_id*phix)','cx_id','z_id','c_id', obj.cx_id,obj.z_id,obj.c_id); % dimensionless
    nyTerm = prepTerm('(-cy_id-z_id*c_id*phiy)','cy_id','z_id','c_id', obj.cy_id,obj.z_id,obj.c_id);

    NxTerm = prepTerm('(D_id*c_ref/L*nx_id)','D_id','nx_id',obj.D_id,obj.nx_id); % dimensional
    NyTerm = prepTerm('(D_id*c_ref/L*ny_id)','D_id','nx_id',obj.D_id,obj.ny_id);

    ixSummand = prepTerm('D_id/D_ref*z_id*nx_id','D_id','z_id','nx_id',obj.D_id,obj.z_id,obj.nx_id); % dimensionless
    iySummand = prepTerm('D_id/D_ref*z_id*ny_id','D_id','z_id','ny_id',obj.D_id,obj.z_id,obj.ny_id);
    ...
    obj.bulkVariables.selection.named('bulkBoundary');
    ...
    obj.upperBoundaryVariables.selection.named('upperBoundary');
...
%     obj.FluxAtSurface.selection.named('bpeSurface');
    ...
    obj.FluxAtBulkBoundary.selection.named('bulkBoundary');
    obj.FluxAtBulkBoundary.label('Flux from bulk');
    obj.FluxAtBulkBoundary.active(false);

    obj.BulkConcentrationsBC.selection.named('lateralBoundary');
...
%     end
    ...
    % n * phi' = 0
    obj.entireSurfacePotentialGradientBC.selection.named('entireSurface');
...
%     obj.insulatorSurfacePotentialGradientBC.selection.named('insulatorAdjacentToBpe');
    obj.insulatorSurfacePotentialGradientBC.selection.named('insulator');
    ...
    obj.bulkBoundaryPotentialGradientBC.selection.named('bulkBoundary');
    ...
    % bulk potential
    obj.bulkPotentialBC.selection.named('lateralBoundary');
...
%     obj.zeroSurfaceCurrent.feature('dode1').set('da', '0');
    ...
    obj.zeroSurfaceCurrentEquation.set('name', 'phi_ref'); 
    obj.zeroSurfaceCurrentEquation.set('equation', 'integrateSurface(i_total)');
    obj.zeroSurfaceCurrentEquation.set('initialValueU', '0');
