...
%     obj.allBoundaries                   = obj.m.selection('allBoundaries');
    obj.bpeSurface                      = obj.m.selection('geom_bpeSurface');
    obj.bulkBoundary                    = obj.m.selection('geom_bulkBoundary');
    obj.zetaPlane                       = obj.m.selection('geom_zetaPlane'); % edge at some distance from BPE (one Debye length), representing the zeta plane
%     obj.lateralBoundaryAtFirstDebyeLength    = obj.m.selection('geom_lateralBoundaryAtFirstDebyeLength'); 
%     obj.lateralBoundaryAtRemainingDomain     = obj.m.selection('geom_lateralBoundaryAtRemainingDomain');
    obj.lateralBoundary                 = obj.m.selection('geom_lateralBoundary');
    obj.upperBoundary                   = obj.m.selection('geom_upperBoundary');
    obj.electrodes                      = obj.m.selection('geom_electrodes');
    obj.workingElectrode                = obj.m.selection('geom_workingElectrode');
    obj.counterElectrode                = obj.m.selection('geom_counterElectrode');
    obj.insulator                       = obj.m.selection('geom_insulator');
    obj.insulatorAdjacentToBpe          = obj.m.selection('geom_insulatorAdjacentToBpe');
    obj.entireSurface                   = obj.m.selection('geom_entireSurface');
%     obj.reactingSurface                 = obj.m.selection('reactingSurface');
%     obj.regionOfFirstDebyeLength        = obj.m.selection('geom_regionOfFirstDebyeLength');
...
%     projectSurfaceToBulkExit = m.cpl('projectSurfaceToBulkExit');
    ...
    obj.upperBoundaryVariables = obj.m.variable('upperBoundaryVariables');
...
%         obj.cSurfaceProbe{i} = obj.m.result.numerical(obj.cSurfaceProbe_id{i});
    ...
    obj.multiPurpose1dPlotGroup = obj.m.result('multiPurpose1dPlotGroup');
    obj.multiPurpose1dPlot      = obj.multiPurpose1dPlotGroup.feature('multiPurpose1dPlot');
